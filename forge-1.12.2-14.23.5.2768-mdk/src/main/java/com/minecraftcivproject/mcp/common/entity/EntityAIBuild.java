...
//        return false;
    ...
    public boolean shouldContinueExecuting() {
        ...
        // also sigh, looks like this can be called before the task is really started.
        if(entityItemFetcher == null){
            return false;
        }
...
//        if (!this.order.isOrderFullfilled()) {
//            logger.info("The order, " + this.order + ", is not filled yet");
//
//            for (int i = 0; i < order.getRemainingRequiredItems(); i++) {
//                if (checkList.get(i)) {
...
//        this.pos = area.searchFor(world, currentBlock, entityPosition);
    ...
    public void updateTask() {
        ...
        // sigh, this seems to get called before the task is initialized
        if(this.entityItemFetcher == null){
            return;
        }
